...
function enterApp() {
  ...
  if (brand && currentRep?.name) brand.textContent = '您好，' + currentRep.name
...
async function renderToday() {
  ...
  let _mgrTimeout = setTimeout(()=>{ c.innerHTML='<div style="padding:40px;text-align:center;color:#999">載入逾時，請重新整理</div>' }, 60000)
...
async function renderCustomers() {
  const c = document.getElementById('page-content')
  c.innerHTML = '<div class="loading"><div class="spinner"></div> 載入中</div>'
  let _custTimeout = setTimeout(()=>{ c.innerHTML='<div style="padding:40px;text-align:center;color:#999">載入逾時，請重新整理</div>' }, 60000)
  ...
  customers.forEach(cu => {
    if (groups[cu.grade]) groups[cu.grade].push(cu)
    else groups['others'].push(cu)
  })

  let html = `<div class="sec-label">我的客戶（${customers.length} 家）</div>`
  ...
  gradeOrder.forEach(({key,label}) => {
    ...
    html += `<div style="font-size:11px;font-weight:600;color:#65655C;letter-spacing:.06em;text-transform:uppercase;margin:14px 0 6px">${label}（${list.length}）</div>`
    list.forEach(cu => {
      const lastVisit = lastVisitMap[cu.id]
      const urg = visitUrgency(lastVisit, cu.visit_interval_days||21)
      html += `<div class="card" style="${gradeLeft(cu.grade)}">
        <div class="card-top">
          <div style="flex:1">
