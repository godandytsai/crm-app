...
window.mgrUploadQuota = async (inp) => {
  ...
  // 解析（簡化版：找月別金額合計）
  const ws = wb.Sheets[qn]
  const raw = XLSX.utils.sheet_to_json(ws, {header:1, defval:null, raw:true})
  let hi = -1
  for (let i = 0; i < Math.min(raw.length, 5); i++) {
    if (raw[i] && raw[i].indexOf('系列') >= 0) { hi = i; break }
  }
  if (hi < 0) { addLog('⚠ 找不到標題列'); inp.value=''; return }

  const hdr = raw[hi]
  const iPe = hdr.indexOf('業務負責人')
  const iReg = hdr.indexOf('區域分類')
  const iUpdCh = hdr.indexOf('更新_客戶分類')
  const iSe = hdr.indexOf('系列'), iCa = hdr.indexOf('分類')
  const mCols = []
  hdr.forEach((v,ci) => { const n=parseFloat(v); if(n>=11501&&n<=11512) mCols.push({ci,mo:String(Math.round(n)%100)}) })
  ...
  for (let ri = hi+1; ri < raw.length; ri++) {
    ...
    const reg = row[iReg] ? String(row[iReg]).trim() : ''
    const se = row[iSe] ? String(row[iSe]).trim() : ''
    const ca = row[iCa] ? String(row[iCa]).trim() : ''
    const isTot = /^(total|TOTAL|Total)$/i.test(se) || /^(total|TOTAL|Total)$/i.test(ca)
    if (!isTot || !curPe || reg === '全通路') continue
    if (!peMonthly[curPe]) peMonthly[curPe] = {}
    mCols.forEach(mc => {
      const val = parseFloat(row[mc.ci]) || 0
      peMonthly[curPe][mc.mo] = (peMonthly[curPe][mc.mo]||0) + val
    })
  }

  const peNames = Object.keys(peMonthly).filter(p => p !== '全通路')
  ...
  peNames.forEach(pe => {
    ...
    Object.entries(peMonthly[pe]).forEach(([mo, amt]) => {
      const moNum = parseInt(mo)
      if (!moNum || moNum < 1 || moNum > 12) return
      const period = year + '-' + String(moNum).padStart(2,'0')
      rows.push({ rep_id: repId, period, amount_target: Math.round(amt), visit_target: 0 })
    })
  })
