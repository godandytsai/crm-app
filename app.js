...
async function renderManagerOverview() {
  ...
  const [
    ...
    sb.from('sales_rep').select('id,name,role').in('role',['sales','manager']).order('name'),
    ...
    sb.from('customer').select('id,name,assigned_rep_id,visit_interval_days,created_at,is_active'),
  ...
  // 依業務彙整本月資料
  const repStats = {}
  ;(reps||[]).filter(r=>r.role==='sales').forEach(r => {
  ...
  // 久未拜訪客戶（超過 visit_interval_days 未被拜訪）
  // 建立 customer_id -> 最後拜訪日
  const lastVisitMap = {}
  ;(allVisits||[]).forEach(v => {
  ...
  const { data: recentVisits } = await sb.from('visit_log')
    .select('customer_id, visited_at').gte('visited_at', new Date(Date.now()-90*86400000).toISOString().slice(0,10)+'T00:00:00')
  ...
  const overdueList = (customers||[]).filter(cu => {
    ...
    const rep = (reps||[]).find(r=>r.id===cu.assigned_rep_id)
    return { ...cu, daysAgo: days, overBy: days ? days - cu.visit_interval_days : null, repName: rep?.name||'未指派' }
  }).sort((a,b) => (b.overBy||999)-(a.overBy||999)).slice(0,20)

  // 本月新增客戶
  const newCusts = (customers||[]).filter(cu => cu.created_at?.slice(0,7)===period)
  ...
  // 業務出勤狀況
  html += `<div class="sec-label">今日出勤（${todayStr}）</div>`
  html += statsArr.map((s,i) => `
    <div class="card" style="display:flex;align-items:center;gap:12px;padding:10px 14px">
      ...
      <div style="flex:1"><div class="card-name">${s.name}</div></div>
    ...
    return `<div class="card">
      <div class="card-top">
        ...
        <div style="flex:1">
          ...
          <div class="card-sub">拜訪 ${s.visits} 次 · 成交 ${s.closed} 筆 · 成交率 ${closeRate}%</div>
        </div>
        <span class="badge ${noteRate>=80?'b-success':noteRate>=50?'b-info':'b-warn'}" title="備註填寫率">${noteRate}% 填</span>
      </div>
  ...
  // 本月新增客戶
  if (newCusts.length) {
    html += `<div class="sec-label" style="margin-top:20px">本月新增客戶（${newCusts.length} 家）</div>`
    html += newCusts.map(cu => {
      const rep = (reps||[]).find(r=>r.id===cu.assigned_rep_id)
      return `<div class="card" style="display:flex;align-items:center;gap:12px;padding:10px 14px">
        <div class="avatar av-green">${initials(cu.name)}</div>
        <div style="flex:1"><div class="card-name">${cu.name}</div><div class="card-sub">${rep?.name||'未指派'} · ${cu.type||''}</div></div>
        <span class="badge b-info">新客戶</span>
      </div>`
    }).join('')
  }
      ...
      <div class="card">
        <div class="card-top">
          ...
          <div style="flex:1">
            ...
            <div class="card-sub">${v.daily_route?.route_date||''} · ${v.daily_route?.rep_id ? ((reps||[]).find(r=>r.id===v.daily_route.rep_id)?.name||'') : ''}</div>
          </div>
  ...
  if (!html.includes('card')) {
    html += '<div class="empty-state"><i class="ti ti-mood-smile"></i>目前沒有待處理事項</div>'
  }
