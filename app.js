...
async function renderManagerOverview() {
  ...
  const day180ago = new Date(Date.now()-180*86400000).toISOString().slice(0,10)  // 6個月
  ...
  ;(recentOrders||[]).forEach(o => {
    ...
    if (ym === prevPeriod) ordersByName[k].thisMonth += o.amount   // 上月當「本期」
    if (ym === prev2Period) ordersByName[k].lastMonth += o.amount  // 上上月當「前期」
  ...
  const noTradeCusts = (customers||[]).filter(cu => {
    ...
    if (!cu.assigned_rep_id || !activeRepIds.has(cu.assigned_rep_id)) return false // 只看三位業務的客戶
  ...
  if (decliningCusts.length) {
    html += `<div class="sec-label" style="margin-top:20px">交易金額下降（${prev2Period} → ${prevPeriod}）</div>`
