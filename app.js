...
async function renderManagerOverview() {
  ...
  // 更新資料管理卡片狀態（從 localStorage 讀）
  try {
    const sm = JSON.parse(localStorage.getItem('sd_smeta')||'{}')
    const entries = Object.entries(sm).filter(([k])=>k!=='supabase')
    const sbMeta = sm.supabase
    let metaStr = ''
    if (sbMeta) metaStr = `已載入 ${sbMeta.rows?.toLocaleString()} 筆 · ${sbMeta.date}`
    else if (entries.length) metaStr = entries.map(([k,v])=>`${v.rows}筆`).join('、')
    else metaStr = '尚未上傳'
    document.getElementById('mgr-sales-meta').textContent = metaStr
    document.getElementById('mgr-sales-badge').textContent = (sbMeta||entries.length) ? '已上傳' : '未上傳'
    document.getElementById('mgr-sales-badge').className = 'badge ' + ((sbMeta||entries.length) ? 'b-success' : 'b-gray')
  } catch(e) {}
}
